        ...
        <>
            <thead>
                <tr className="pb-[7px] border-b border-[#585858] text-table-header text-dark-100 flex gap-5">
                    <th className="w-[25%]">Destination Address</th>
            ...
            <tbody className="grow overflow-y-auto">
                {data.map((transaction) => (
                    ...
                    >
                        <td className="w-[25%] text-light-200">{transaction.destinationAddress}</td>
                        <td className="w-[10.5%]">{transaction.asset || "XLM"}</td>
                        <td className="w-[12%] text-indicator-500">{moneyFormat(transaction.amount)}</td>
